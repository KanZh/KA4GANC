...
class KA4GANC(nn.Module):
    ...
    def encode(self,x,adj):
        ...
        x = self.ConvLayer1(x,adj)
        x = F.leaky_relu(x)

        x = self.ConvLayer12(x,adj)
        x = F.leaky_relu(x)

        x = self.ConvLayer123(x,adj)
        x = F.leaky_relu(x)
...
class Conv(nn.Module):
    def __init__(self,in_channels,out_channels,kernel_size,hidden_dim,stride=1,padding=0,alpha=0.2,bias=True):
        ...
        self.conv = KANConvLayer(in_channels, out_channels, kernel_size, groups=1)
